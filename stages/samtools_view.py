...
class samtools_view(stage_wrapper.Stage_Wrapper):
    ...
    def run(self,run_id,inputs):
        ...
        #self.db_start(run_id,in_names['.sam'])
        ...
        if err == {}:
            ...
            if all([os.path.exists(r) for r in results]):
                print("<<<<<<<<<<<<<samtools view sucessfull>>>>>>>>>>>>>>>\n")
                return results   #return a list of names
            else:
                print("<<<<<<<<<<<<<samtools view failure>>>>>>>>>>>>>>>\n")
                return False
        else:
