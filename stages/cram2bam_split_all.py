...
#function for auto-making svcpdb stage entries and returning the stage_id
class cram2bam_split_all(stage_wrapper.Stage_Wrapper):
    ...
    def run(self,run_id,inputs):
        ...
        try:
            ...
            samtoolsin  = subprocess.Popen(' '.join(cram2sam),stdout=subprocess.PIPE,shell=True)
            ...
            for line in samtoolsin.stdout:
                ...
                if(read.startswith("@")):
                    for h in H:
                        savesams[h].write(read+'\n')
                else:
                    linesplit = read.split('\t') #alignment reacords are length 11
                    i = tuple(sorted([linesplit[2],linesplit[6]])) #get sorted tuple refs for pairs
                    if savesams.has_key(K[i]):
                        savesams[K[i]].write(read+'\n')
            for h in savesams:
                savesams[h].close()
            for h in bams:
