...
parser.add_argument('-z', '--coor', type=str, help='reference coordinate map for writing vcf or g1k output files\t[None]')
parser.add_argument('-y', '--sv_mask',type=str,help='user supplied svmask file in BED3 format\t[None]')
args = parser.parse_args()

if args.coor is None:
    print('no coordinate offset map specified')
    raise IOError
...
if args.chroms is not None:
    ...
    chroms = ru.get_chroms(args.coor)
...
# This is a data structure we set up to catch exception information from child threads
manager = mp.Manager()
exception_bucket = manager.Queue()
...
def partition_call_sets(sample,k,O,R,B,chroms,flt,flt_exclude,caller_exclude):
    ...
    #         print traceback.format_exc()
        ...
        # print "Found the error" # DEBUG
        # print str(exception_bucket.qsize()) # DEBUG
        exc_type, exc_obj, exc_trace = sys.exc_info()
        # exception_bucket.put(exc_type)
        # exception_bucket.put(exc_obj)
        exception_bucket.put(traceback.format_exc(exc_trace))
        # print str(exception_bucket.qsize()) # DEBUG
#[1] File Partitioning------------------------------------------------------------------
...
if __name__ == '__main__':
    ...
    R = []
    #mask these regions------------------------------------------------------------------------------------
    if args.sv_mask is not None:
        R += ru.get_mask_regions(args.sv_mask,O)               #svmask from ref complexity
...
#    success = fusor.write_partitions_by_sample(sname_partition_path,P)
    ...
    if total_partitions<1: #skip this step if you have already read and partitioned the data set      
        ...
        return_list = []
        for sample in samples:
            return_list.append(p1.apply_async(partition_call_sets,
                           args=(sample,k,O,R,B,chroms,flt,[],exclude_callers),
                           callback=collect_results))
            # result_list.append(partition_call_sets(sample,k,O,R,B,chroms,flt,[],exclude_callers))
            time.sleep(0.25)
        p1.close()
        p1.join()
        # Check for exceptions in the threads
        if not exception_bucket.empty():
            # exc_type = exception_bucket.get(block=False)
            # exc_obj = exception_bucket.get(block=False)
            exc_trace = exception_bucket.get(block=False)
            # print "Neb2" # DEBUG
    ...
    for n_k in range(n_cross): #default is 1 and cross_fold = 0
        ...
        for sample in [samples[i] for i in tst_ids]:
            return_list.append(p1.apply_async(apply_model_to_samples,
                           ...
                           callback=collect_results))
            # result_list.append(apply_model_to_samples(sample,ref_path,chroms,types,bins,callers,O,model_path,apply_fusion_model_path,k,f_id,over_m,0.5,args.brkpt_smoothing,True,True,6));
            time.sleep(0.25)
        p1.close()
        p1.join()
        # Check for exceptions in the threads
