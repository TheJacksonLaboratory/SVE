...
SVE uses a Object Orientation pattern and heavy encapsulation to decrease the percieved complexity and configuration of arbitrary pipelines.  The heart of the system in vokes the registration of a stage which is extended by inheriting from the stage_wrapper class.  A stage is written by this extension and then a run(inputs) method must be defined or overloaded.  Execution usually start by collecting and passing arguments into the run runing stage, but before this occurs the API can be configured with a database that can access frequent information such as reference identifies (using MD5 hashes of the sequence lengths) or other mechanisms.  Each stage has a unique intiger identifier that allows the databases or SVEDB to keep track of runtines for each file which can be used to gather information such as the avergae runtime for SV calling on a sample ect.  Once a stage has been registered at runtime, the name is used to load the configurations at runtime which includes a stage param_map (JSON file) that passes valuable information such as the window size to use, ect.  Once the run method is envoked, the SVE spawns a new process on the host system and puts in a robust block on the SVE section that was run to that point.  Each stage is run inside this child process so that the output is fully checked meaning that if the stages fails, the SVE does not and can rerun stages or continue to the next stage in a list ect...An example of this workflow can be seen inside the prepare_bam.py and variant_processor.py scripts which dynamically load stages as needed.<br>
##Usage
